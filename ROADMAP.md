...
### Round 14 — Four new phase types + perchance_1 rewrite
...
| **perchance_1.txt rewrite** | Top editor went from 99 → 400 lines. Updated $meta with current accurate description (mentions Smart Director, programs, opt-in phase types). Inline catalogs in $meta.dynamic now match the actual METHODS/INTENTIONS/PRESETS/PROGRAMS arrays — previously stale (missed dave-elman, regression, somnambulism, all the new presets, etc.). Added route handlers for `/?preset=<id>` and `/?program=<id>` URL routes for richer link previews |
| New plugin imports in perchance_1 | `superFetch` (CORS-bypassed external fetch — useful for user-pasted background image URLs that browsers would block) and `bugReport` (packages browser/device debug info into one-click issue submission). Both imported optionally; HTML panel guards via `typeof root.X !== 'undefined'` so unavailable plugins degrade gracefully |
| `freshSeeds` library in perchance_1 | New `freshSeedsBank` Perchance list with 50+ tagged anchor phrases across 12 categories (sleep, anxiety, performance, focus, creativity, pain, trance, confidence, behavior, grounding, etc.). Each entry has a tag (multiple via pipe `\|` separator) and a sensory phrase. `root.freshSeeds.pick({profileTags, n, excludeSet})` filters by tag overlap, falls back to any phrase if filtered set is too small, Fisher-Yates shuffles, returns up to n. HNE's Smart Director already calls this — was previously returning empty. Now Smart Director gets fresh metaphor anchors per session, reducing AI's tendency to converge on the same imagery (staircase, warm light, deep ocean) for similar requests |
| `knownProject` metadata exposed | New Perchance list exposes name/version/phaseTypes/lengthBuckets/optInPhaseTypes for the HTML panel to read. Wired into HNE diagnostic shim via new `HNE.project()` method — useful for sanity-checking top-editor ↔ HTML-panel version sync when filing bug reports |
| Better `communityPacks` documentation | Empty placeholder remains, but with detailed inline docs explaining the four pack types (personas, methods, presets, programs), the data shapes each must export, the publish workflow (create generator → add id to slot), and the consume pattern (HTML panel reads `root.communityPacks.<slotName>.<itemType>`). Four commented example slots show the syntax |
